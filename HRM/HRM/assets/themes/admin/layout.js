...
$(document).ready(function() {
    ...
    $(function() {
        ...
        if (path !== undefined) {
            if (path == "") {
                $("#sidebar-menu").find("a[href$='/']").addClass('sfActive');
                $("#sidebar-menu").find("a[href$='/']").parents().eq(3).superclick('show');
            }
...
/* Breadcrumb Menu  */
    //HieuND 
    $(function () {
        var name = $("#sidebar-menu").find("a.sfActive").find("span").html(); //Lấy tên của Menu đang active
        console.log(name);
        if (name == undefined) { // Check null
            $('.breadcrumb-contain').css('display', 'none'); // Nếu menu chưa chọn ( trên Login ) thì ko hiện breadcrumb
        }
        $('.breadcrumb-contain .breadcrumb').append('<li class="active">'+name+'</li>'); //Thêm các menu đang trỏ tới
